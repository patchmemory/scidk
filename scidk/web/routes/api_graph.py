...
@bp.get('/schema/map')
def api_schema_map():
    """Get schema visualization data for Cytoscape rendering.

    Returns:
        JSON with Cytoscape-compatible format:
        {
            nodes: [{data: {id, label, count, description, color}}],
            edges: [{data: {id, source, target, label, count}}]
        }
    """
    try:
        # Get primary Neo4j driver
        driver = None
        try:
            from neo4j import GraphDatabase
            uri, user, password, database, auth_mode = get_neo4j_params()
            if uri:
                driver = GraphDatabase.driver(uri, auth=None if auth_mode == 'none' else (user, password))
        except Exception as e:
            return jsonify({"error": f"Failed to connect to Neo4j: {str(e)}"}), 500

        if not driver:
            return jsonify({"error": "Neo4j not configured"}), 400
            ...
            with driver.session(database=database) as session:
                ...
                try:
                    ...
                    for record in result:
                        ...
                        # Try to get description from label_profile
                        description = ""
                        color = "#3498db"  # Default blue
                        try:
                            from ...services.label_service import LabelService
                            label_service = LabelService()
                            profile = label_service.get_label_profile(label)
                            if profile and 'description' in profile:
                                description = profile['description']
                            # Color coding by count (simple heuristic)
                            if count > 1000:
                                color = "#e74c3c"  # Red for high count
                            elif count > 100:
                ...
                RETURN relationshipType, value.source as source, value.target as target, value.count as count
                """
                try:
                    ...
                    # Fallback without APOC (slower but works)
                    ...
                    for record in rel_types_result:
                        rel_type = record['relationshipType']
                        # Sample relationships to find source/target labels
                        sample_query = f"""
